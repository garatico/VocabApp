...
from .config import (
    ...
    WIKT_LANG, TENSE_MAP, MLCONJUG3_LANG, LANGS_WITHOUT_CONJUGATION,
    curated_path, gloss_cache_path, seed_path,
...
def infer_conjugation_class(lang: Lang, infinitive: str) -> Optional[str]:
    ...
    rules engine in src/server/lib/verb-rules.ts. French, Italian and Portuguese
    get a full conjugation table from mlconjug3 at sync time instead, so leaving
    this null is correct rather than a gap.

    German is the exception to both: no rules engine and no mlconjug3 support,
    so its verbs currently carry neither. See LANGS_WITHOUT_CONJUGATION.
    """
    if lang.code != 'spa':
        return None
...
def mine_lang(
    ...
    # An empty list used to return early here, which made a language with no
    # curated file yet impossible to bootstrap: mine would report nothing and
    # exit, and the only way to get a first entry was to hand-write one. Zero
    # existing entries is the correct starting state for a new language, so it
    # is now a normal run that happens to skip nothing.
    curated = read(lang.code)
    if not curated:
        print(f'  Curated    : none yet — bootstrapping {lang.name} from rank 1')
        print(f'               (will create {lang.curated_path.name} on --write)')
...
# ══════════════════════════════════════════════════════════════════════════════
# SEED — hand-written word list -> curated entries
# ══════════════════════════════════════════════════════════════════════════════
# The manual counterpart to `mine`. Mining needs spaCy and a network connection
# for glosses; seeding needs neither, so a language can be given a usable core
# vocabulary before either is available — which is how German was bootstrapped.
#
# Only the words and their glosses are hand-written. Every number (rank, band,
# difficulty, corpus_frequency) is read from the OpenSubtitles corpus here, so
# the seed file cannot drift out of step with the frequency data.
...
SEED_POS = {'noun', 'verb', 'adjective', 'adverb'}

# Not every language cuts gender the same way. Spanish and the other Romance
# languages have two, German three, and Dutch has de-words and het-words —
# 'common' (the merged historical masculine and feminine) and 'neuter'. All the
# valid values live in one set rather than one per language, because a noun
# carrying the wrong system's label is the mistake worth catching, and that
# shows up as an unknown value either way.
SEED_GENDERS = {'masculine', 'feminine', 'neuter', 'common'}


def read_seed(lang_code: str) -> Tuple[List[SeedWord], List[str]]:
    """
    Parse data/seed/<language>.txt.  Returns (words, problems).

    Format is `word|pos|gender|gloss;gloss`. Malformed lines are reported by
    line number rather than skipped silently — a typo in a hand-written file
    should be visible, not quietly drop a word.
    """
    path = seed_path(lang_code)
    words: List[SeedWord] = []
    problems: List[str] = []
    if not path.exists():
        return words, [f'no seed file at {path}']
    ...
    with open(path, encoding='utf-8') as f:
        for lineno, raw in enumerate(f, 1):
            ...
            parts = line.split('|')
            if len(parts) != 4:
                problems.append(f'line {lineno}: expected 4 fields, got {len(parts)}')
                continue
            word, pos, gender, gloss_str = (p.strip() for p in parts)
            glosses = [g.strip() for g in gloss_str.split(';') if g.strip()]

            if not word:
                problems.append(f'line {lineno}: empty word'); continue
            if pos not in SEED_POS:
                problems.append(f'line {lineno}: {word!r} has pos {pos!r}, expected one of {sorted(SEED_POS)}')
                continue
            if gender and gender not in SEED_GENDERS:
                problems.append(f'line {lineno}: {word!r} has gender {gender!r}')
                continue
            if pos == 'noun' and not gender:
                problems.append(f'line {lineno}: noun {word!r} has no gender')
            if not glosses:
                problems.append(f'line {lineno}: {word!r} has no glosses'); continue

            key = word.lower()
            if key in seen:
                continue            # a duplicate in the seed file is harmless
            seen.add(key)
            words.append(SeedWord(word, pos, gender or None, glosses))

    return words, problems


def _check_pos_with_hanta(words: List[SeedWord], lang_code: str) -> List[str]:
    """
    Cross-check the hand-written part of speech against HanTa, if installed.

    Optional — the seed step works without it, this just catches mistakes.
    HanTa is used rather than spaCy because its German model ships inside the
    wheel, so it needs no separate download.

    GERMAN ONLY. HanTa has one model and it is German; handing it Dutch would
    tag every word against German morphology and report a wall of disagreements
    that mean nothing. A check that cannot tell right from wrong is worse than
    no check — it trains you to ignore the output.

    Nouns are checked in their capitalised form. Handed a lowercase noun the
    tagger returns whichever verb or adjective it collides with — frage for
    Frage, liebe for Liebe, arm for Arm — which would report every one of them
    as a mistake.
    """
    if lang_code != 'deu':
        return []
    ...
    tags = {'NN': 'noun', 'NE': 'noun', 'VV': 'verb', 'VA': 'verb',
            'VM': 'verb', 'ADJ': 'adjective', 'ADV': 'adverb'}

    out: List[str] = []
    for w in words:
        probe = w.word if w.pos == 'noun' else w.word.lower()
        try:
            _, tag = tagger.analyze(probe, taglevel=1)
        except Exception:
            continue
        got = tags.get(tag.split('(')[0])
        if got and got != w.pos:
            out.append(f'{w.word}: seed says {w.pos}, HanTa says {got}')
    return out


def seed_lang(lang_code: str, existing: List[dict]) -> Tuple[List[dict], dict]:
    """
    Turn the seed file into curated entries.

    Words already in the curated file are skipped, so this is safe to re-run
    and safe to run after mining. Nothing is written — pipeline.py persists.

    Returns (new_entries, report).
    """
    seeds, problems = read_seed(lang_code)
    report = {'seeds': len(seeds), 'problems': problems, 'skipped': 0,
              'no_corpus': [], 'pos_disagreements': []}
    if not seeds:
        return [], report

    have = {(e.get('word') or '').lower() for e in existing}
    fresh = [s for s in seeds if s.word.lower() not in have]
    report['skipped'] = len(seeds) - len(fresh)
    if not fresh:
        return [], report

    # Real frequencies, so the seed file never has to carry a number.
    ranks = corpus.load_os_ranks(lang_code, OS_DIR)
    report['no_corpus'] = [s.word for s in fresh if s.word.lower() not in ranks]
    report['pos_disagreements'] = _check_pos_with_hanta(fresh, lang_code)

    def corpus_count(s: SeedWord) -> int:
        return (ranks.get(s.word.lower()) or {}).get('count', 0)

    # Ordered by how common the word actually is, so "Top 100" in the app means
    # the hundred commonest of them.
    fresh.sort(key=corpus_count, reverse=True)

    next_rank = max((e.get('rank') or 0 for e in existing), default=0) + 1
    entries: List[dict] = []
    for offset, s in enumerate(fresh):
        rank = next_rank + offset
        info = ranks.get(s.word.lower()) or {}
        entry = build_curated_entry(
            lang=make_lang(lang_code),
            word=s.word,
            pos=s.pos,
            corpus_rank=info.get('rank') or rank,
            corpus_count=info.get('count') or 0,
            glosses=s.glosses,
            curated_rank=rank,
            gender=s.gender,
        )
        entry['source'] = 'handcurated'
        entry['tags']   = ['handcurated', 'core']
...
def fill_conjugations(entries: List[dict], lang: str) -> int:
    ...
    # Say it once, here, rather than letting mlconjug3 fail per verb further
    # down. German has no conjugator: its verbs are imported with
    # conjugations: null, which is a known gap rather than a malfunction.
    if lang in LANGS_WITHOUT_CONJUGATION:
        verbs = sum(1 for e in entries if e.get('pos') == 'verb')
        print(f'  Conjugations : not available for {LANG_NAMES.get(lang, lang)} — '
              f'{verbs} verbs imported without them')
        print('                 (mlconjug3 covers en/es/fr/it/pt/ro only; German '
              'needs a rules engine of its own,')
        print('                  the way Spanish has one in verb-rules.ts)')
        return 0
